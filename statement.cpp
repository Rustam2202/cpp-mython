...
namespace ast {
	...
	VariableValue::VariableValue(std::vector<std::string> dotted_ids) {
		if (auto size = dotted_ids.size(); size > 0) {
			...
			std::move(dotted_ids.begin() + 1, dotted_ids.end(), dotted_ids_.begin());
		}
	}
		...
		if (closure.count(var_name_)) {
			...
			if (dotted_ids_.size() > 0) {
				if (auto obj = result.TryAs<runtime::ClassInstance>()) {
					...
					throw std::runtime_error("Variable is not class"s);
				}
			}
			return result;
		}
	...
	ObjectHolder MethodCall::Execute(Closure& closure, Context& context) {
		if (auto class_instance = object_->Execute(closure, context).TryAs<runtime::ClassInstance>()) {
	...
	ObjectHolder Add::Execute(Closure& closure, Context& context) {
		...
		auto l_numb = left.TryAs<runtime::Number>();
		auto r_numb = right.TryAs<runtime::Number>();
		auto l_str = left.TryAs<runtime::String>();
		auto r_str = right.TryAs<runtime::String>();
		if (l_numb && r_numb) {
			return ObjectHolder::Own(runtime::Number(l_numb->GetValue() + r_numb->GetValue()));
		}
		else if (l_str && r_str) {
			return ObjectHolder::Own(runtime::String(l_str->GetValue() + r_str->GetValue()));
		}
		if (auto left_class = left.TryAs<runtime::ClassInstance>()) {
			return left_class->Call(ADD_METHOD, { right }, context);
		}
		throw std::runtime_error("Can add only numbers, strings and class instances"s);
	}
	...
	ObjectHolder Comparison::Execute(Closure& closure, Context& context) {
		auto result = comp_(lhs_->Execute(closure, context), rhs_->Execute(closure, context), context);
